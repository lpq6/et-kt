...
class MultiModeResidualInputMemorySSM(InputMemorySSM):
    ...
    def from_existing(cls, source, mode_count=8, enabled=True):
        if type(source) is not InputMemorySSM:
            raise TypeError(
                "the modal residual requires the retained V39 input-memory SSM"
            )
        if type(mode_count) is not int or not 2 <= mode_count <= 8:
            raise ValueError(
                "supported synthetic/production mode counts are 2 through 8"
            )
        ...
        module.register_buffer(
            "rate_multipliers", torch.exp2(-powers), persistent=False
        )
        return module
    ...
    def forward(self, x, scope_weight=None, concept_prior=None):
        ...
        modal_hidden = x.new_zeros(x.size(0), x.size(-1), self.mode_count)
        reference_hidden = x.new_zeros(x.size(0), x.size(-1), self.mode_count)
        outputs = []
        ...
        for index in range(x.size(1)):
            ...
            modal_retention = retention.unsqueeze(-1).pow(self.rate_multipliers)
            writes = 2.0 * torch.sigmoid(F.linear(x[:, index], self.mode_input_weight))
            reads = torch.softmax(F.linear(x[:, index], self.mode_read_weight), dim=-1)
            modal_drive = candidate.unsqueeze(-1) * writes.unsqueeze(1)
            modal_hidden = (
                modal_retention * modal_hidden + (1.0 - modal_retention) * modal_drive
            )
            reference_hidden = modal_retention * reference_hidden + (
                1.0 - modal_retention
            ) * candidate.unsqueeze(-1)
            reference_reads = x[:, index].new_full(
                (x.size(0), self.mode_count), 1.0 / self.mode_count
            )
            modal_readout = (modal_hidden * reads.unsqueeze(1)).sum(dim=-1)
            reference_readout = (reference_hidden * reference_reads.unsqueeze(1)).sum(
                dim=-1
            )
            if scope_weight is not None:
                gate = gate * scope_weight[:, index]
            original_output = gate * hidden + (1.0 - gate) * x[:, index]
            modal_output = gate * modal_readout + (1.0 - gate) * x[:, index]
            reference_output = gate * reference_readout + (1.0 - gate) * x[:, index]
