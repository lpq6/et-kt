"""CPU-only, cache-free train/validation sequences with fixed-order metadata."""
...
FIELDS = {
    "questions": "qseqs", "concepts": "cseqs", "responses": "rseqs",
    "timestamps": "tseqs", "usetimes": "utseqs", "selectmasks": "smasks",
}
...
class TrainValidationDataset(Dataset):
    def __init__(self, path, config, folds):
        ...
        if not folds or not folds <= {0, 1, 2, 3, 4} or (0 in folds and len(folds) != 1):
        ...
        selected_fields = [
            field for field in FIELDS if field in fieldnames
            and (field not in ("questions", "concepts") or field in config["input_type"])
        ]
        buffers = {
            field: np.empty((row_count, config["maxlen"]), dtype=(
                np.float32 if field == "responses" else np.int64
            ))
        ...
        valid = selection == 1
        for field, capacity in (("concepts", config["num_c"]), ("questions", config["num_q"])):
            ...
            if np.any(identifiers[valid] < floor) or np.any(identifiers[valid] >= capacity):
        ...
        self.dori = {
            name: torch.from_numpy(buffers[field]) if field in buffers else torch.empty(0, dtype=torch.long)
            for field, name in FIELDS.items()
        }
        self.dori["masks"] = (
            (self.dori["cseqs"][:, :-1] != -1) & (self.dori["cseqs"][:, 1:] != -1)
        )
...
class FixedOrderBatchSampler(Sampler):
    def __init__(self, metadata, batch_size, seed, epoch):
        ...
        self.signature = hashlib.sha256(json.dumps({
            ...
            "order": [[entry.csv_row_index, entry.learner_uid] for entry in metadata],
    ...
    def state_dict(self):
        return {
            "schema_version": 1, "seed": self.seed, "epoch": self.epoch,
            "batch_size": self.batch_size, "batch_count": len(self),
            "schedule_sha256": self.signature, "order": "fixed_csv_filtered_order",
