...
from work.a2g_mambakt_one_epoch_concept_novelty_curriculum_candidate import (
    A2GMambaKT as _ConceptNoveltyA2GMambaKT,
...
class A2GMambaKT(_ConceptNoveltyA2GMambaKT):
    """Remove two weak statistics from learned paths, retaining direct evidence."""

    def __init__(self, *args, use_joint_weak_stat_prune=1, **kwargs):
        super().__init__(*args, **kwargs)
        self.use_joint_weak_stat_prune = bool(int(use_joint_weak_stat_prune))

    def _learned_stats(self, stats):
        if not self.use_joint_weak_stat_prune:
            return stats
        learned = stats.clone()
        learned[..., 5] = 0.0
        learned[..., 7] = 0.0
